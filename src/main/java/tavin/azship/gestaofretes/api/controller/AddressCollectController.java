...
public class AddressCollectController {
    ...
    @PostMapping
    public ResponseEntity<AddressCollect> create(@Valid  @RequestBody AddressCollectDTO dto){
        return new ResponseEntity<>(collectService.create(dto), HttpStatus.CREATED);
    }

    @PutMapping("/{id}")
    public ResponseEntity<AddressCollect> update(@PathVariable Long id,@Valid @RequestBody AddressCollectUpdateDTO dto){
